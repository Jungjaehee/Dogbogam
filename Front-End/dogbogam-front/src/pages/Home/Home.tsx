...
import useDogStore, { DogInfo } from "../../store/UseDogStore";
...
export const Home = () => {
  ...
  const {dogInfo, setDogInfo} = useDogStore();

  const handleModalClose = () => {
    setIsModalOpen(false);
  }

  const handleConfirm = (dog: Partial<DogInfo>) => {
    setIsModalOpen(false);
    setDogInfo(dog);
  }
  ...
  return (
    <div className="h-full pt-6 px-4 bg-white flex flex-col justify-between">
      <div className="h-full flex flex-col">
        <div 
        className="w-32 h-14 flex items-center space-x-2 bg-white rounded-lg shadow-md mt-3 mb-7">
          <DogCard dog={dogInfo} />
          <img
            src={Down}
        ...
        <div className="mb-7">
          <p className="text-gray-800 text-xl font-extrabold">
            오늘의 <span className="text-main-color">{`${dogInfo.name}`}</span>는, 
            <br />
            활기찬 산책을 하고 왔나요?
          </p>
        </div>
        ...
        <div className="mb-7 w-full h-auto flex space-x-4">
          <div className="p-5 w-1/2 h-40 bg-main-color rounded-lg shadow-lg flex flex-col justify-between" 
          ...
          <div className="p-5 w-1/2 h-40 bg-[#56ae75] rounded-lg shadow-lg flex flex-col justify-between" 
          onClick={() => {navigate("/home/supplement")}}
          style={{cursor:"pointer"}}>
