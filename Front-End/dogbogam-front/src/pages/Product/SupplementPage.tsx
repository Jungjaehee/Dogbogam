...
export const SupplementPage = () => {
  ...
  const searchedSupplementList = supplementList.filter((item) =>
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="mb-6">
          <p className="text-gray-800 text-xl font-bold">
            <span className="text-main-color">{`${dogInfo.name}`}</span>를 위한, <br /> 영양제
          </p>
        </div>
        <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {searchedSupplementList.map((supplement) => (
            <div
              key={supplement.supplementId}
              className="border border-gray-100 rounded-lg shadow-lg p-5"
            >
              <div
              ...
              >
                <img src={supplement.imageUrl} alt="" className="w-full h-[80px]" />
        ...
        {isModalOpen && selectedSupplement && (
          <SupplementDetailModal supplementDetail={selectedSupplement} onClose={closeModal} />
        )}
      </div>
    </div>
  );
